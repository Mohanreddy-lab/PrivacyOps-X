...
def dashboard() -> str:
    ...
    before_html = _render_trajectory(payload.get("before_behavior"))
    after_html = _render_trajectory(payload.get("after_behavior"))

    tmpl = _read_template("dashboard.html")
    if tmpl:
        return (
            tmpl
            .replace("$random_score", random_score)
            .replace("$teacher_score", teacher_score)
            .replace("$baseline_score", baseline_score)
            .replace("$improved_score", improved_score)
            .replace("$sft_score", sft_score)
            .replace("$sft_label", sft_label)
            .replace("$random_plot_html", random_plot_html)
            .replace("$self_plot_html", self_plot_html)
            .replace("$before_html", before_html)
            .replace("$after_html", after_html)
        )
